...
--[后台QQ微信红包提醒]
function QMPlugin.REDenvelop()   
local Text = QMPlugin.ticker()
local WR = string.match(Text,".+=(.+): %[微信红包%]")  logcat(WR)
if WR then
We = string.match(Text,"%[微信红包%]([^\r\n]+)") logcat(We)
if We == nil then We = "" end
end
local QR = string.match(Text,".+=(.+): %[QQ红包%]") logcat(QR)
if QR then
Qe = string.match(Text,"%[QQ红包%]([^\r\n]+)") logcat(Qe)
if Qe == nil then Qe = "" end
end
return WR,We,QR,Qe 
end
